...
#define EUDELAY_UI_URI EUDELAY_URI "#ui"
...
static const ControlDesc kControls[] = {
    { "SCALE", PORT_SCALE, 0.125f, 8.0f, 1.0f, false },
    { "STEPS", PORT_STEPS, 2.0f, 24.0f, 16.0f, true },
    { "TAPS", PORT_TAPS, 1.0f, 24.0f, 5.0f, true },
    { "OFFSET", PORT_OFFSET, 0.0f, 24.0f, 0.0f, true },
    { "FB", PORT_FEEDBACK, 0.0f, 100.0f, 35.0f, true },
    { "MIX", PORT_MIX, 0.0f, 100.0f, 35.0f, true }
};
...
static Knob* find_knob_by_port(PMixUI* ui, uint32_t port) {
    if (!ui) {
        return NULL;
    }
    for (int i = 0; i < ui->knob_count; ++i) {
        if (ui->knobs[i].port == port) {
            return &ui->knobs[i];
        }
    }
    return NULL;
...
static void enforce_linked_ranges(PMixUI* ui) {
    Knob* steps = find_knob_by_port(ui, PORT_STEPS);
    Knob* taps = find_knob_by_port(ui, PORT_TAPS);
    Knob* offset = find_knob_by_port(ui, PORT_OFFSET);
    if (!steps || !taps || !offset) {
        return;
    }

    taps->max = steps->value;
    offset->max = steps->value;

    const float new_taps = clamp_value(taps, taps->value);
    if (fabsf(new_taps - taps->value) > 0.0001f) {
        taps->value = new_taps;
        update_edit_text(taps);
        send_value(ui, taps);
    }

    const float new_offset = clamp_value(offset, offset->value);
    if (fabsf(new_offset - offset->value) > 0.0001f) {
        offset->value = new_offset;
        update_edit_text(offset);
        send_value(ui, offset);
    }
}
...
static void draw_ui(PMixUI* ui) {
    ...
    cairo_show_text(cr, "EUDELAY RHYTHMIC DELAY");
...
static LV2UI_Handle ui_instantiate(const LV2UI_Descriptor* /*descriptor*/,
    ...
    if (!parent) {
        fprintf(stderr, "eudelay-ui: no parent window provided\n");
        free(ui);
        return NULL;
    }
    ...
    XStoreName(display, ui->window, "EuDelay");
