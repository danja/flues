...
#define LOG_PREFIX "[Floozy Plugin] "
...
static void apply_parameters(FloozyLV2* self) {
    if (!self || !self->engine) {
        return;
    }
...
static void handle_midi(FloozyLV2* self, const uint8_t* msg, uint32_t size) {
...
static LV2_Handle instantiate(const LV2_Descriptor*, double rate,
    ...
    using namespace flues::floozy;

    auto* self = new FloozyLV2();
...
static void cleanup(LV2_Handle instance) {
    auto* self = static_cast<flues::floozy::FloozyLV2*>(instance);
    delete self;
}

static void connect_port(LV2_Handle instance, uint32_t port, void* data) {
    using namespace flues::floozy;
    auto* self = static_cast<FloozyLV2*>(instance);
