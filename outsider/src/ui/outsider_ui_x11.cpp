...
public:
    ...
    void draw() {
        ...
        draw_text(cr, 140, 30, "Scaffold build: transport/session shell with stub networking", 12.0, 0.73, 0.78, 0.84, false);
        ...
        for (const EndpointRecord& endpoint : registry.endpoints()) {
            ...
            draw_text(cr, 710, row_y, target_state_name(endpoint.current_state), 12.0, 0.92, 0.92, 0.96, false);
        ...
        if (server.recent_events().empty()) {
            ...
            for (const std::string& event : server.recent_events()) {
                draw_text(cr, 34, log_y, event.c_str(), 12.0, 0.90, 0.90, 0.94, false);
                log_y += 24.0;
            }
        }

        if (!registry.endpoints().empty()) {
