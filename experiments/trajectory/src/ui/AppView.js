import { TrajectoryEngine } from '../audio/TrajectoryEngine.js';
import { TRAJECTORY_PARAMS, getDefaultParamState, resolveParamValue } from '../audio/TrajectoryParams.js';
...
export class AppView {
  ...
  mount() {
    ...
    this.updateTrajectoryParams(getDefaultParamState());
    ...
    this.container.innerHTML = `
      <div class="app">
        <header class="app__header">
          <h1 class="app__title">Trajectory Polygon Synth</h1>
        ...
        <section class="panel trajectory-panel">
          <div class="trajectory-panel__header">
            <div>
              <h2 class="panel__title">Trajectory Oscillator</h2>
              <p class="panel__subtitle">Bouncing point inside a regular polygon</p>
            </div>
            <div class="oscilloscope">
              <canvas data-oscilloscope></canvas>
            </div>
          </div>
          <div class="controls-grid" data-trajectory-controls></div>
        </section>
  ...
  setupTrajectoryControls() {
    const container = this.container.querySelector('[data-trajectory-controls]');
    container.innerHTML = '';
    this.paramKnobs.clear();

    const defaults = getDefaultParamState();
    ...
    TRAJECTORY_PARAMS.forEach((param) => {
      ...
      const normalized = defaults[param.id] ?? param.default ?? 0.5;
      const resolved = resolveParamValue(param.id, normalized);
      ...
      const knobControl = new KnobControl({
        ...
        onInput: (value) => {
          this.engine.setParam(param.id, value);
        },
        formatDisplay: (norm) => resolveParamValue(param.id, norm).formatted,
      });

      this.paramKnobs.set(param.id, { control: knobControl, valueElement });
    });
  }
  ...
  setupVisualizer() {
    const canvas = this.container.querySelector('[data-oscilloscope]');
    if (!canvas) return;
  ...
  handleEngineState(state) {
    ...
    this.updateTrajectoryState(state);

    if (this.visualizer) {
      if (state.contextState === 'running') {
        if (!this.visualizer.isRunning) {
          this.visualizer.start(() => this.engine.getAnalyserData());
        }
      } else {
  ...
  updateTrajectoryState(state) {
    if (!state?.params) return;
    TRAJECTORY_PARAMS.forEach((param) => {
      const knob = this.paramKnobs.get(param.id);
      if (!knob) return;

      const resolved = state.params[param.id];
      if (!resolved) return;

      knob.control.setNormalized(resolved.normalized, { emit: false });
    });
  }

  updateTrajectoryParams(params) {
    TRAJECTORY_PARAMS.forEach((param) => {
      const knob = this.paramKnobs.get(param.id);
      if (!knob) return;
  ...
  showError(error) {
    ...
    window.dispatchEvent(new CustomEvent('trajectory:error', { detail: error }));
  }
}
