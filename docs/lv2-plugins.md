...
### gremlin
Gremlin is a glitch-heavy malfunction instrument built around unstable digital oscillators, chaotic modulation, and a delay network that is designed to wobble, smear, and threaten runaway feedback. It has a dedicated MIDImix performance layer, an X11/Cairo UI that mirrors the controller strip layout, and status feedback for mode, scene, macros, and button layers even if the hardware LEDs are unreliable.

*One of the more immediately playable oddballs in the repo. Good for broken synth stabs, unstable drones, and delay-driven accidents.*
...
### convulse
Convulse is an experimental stereo convolution effect whose impulse kernel is generated by a small built-in synth instead of loaded from a file. It renders short FIR kernels from waveform modes such as Sine, Saw, Pulse, Noise, FM, and Chirp, then convolves incoming mono or stereo audio against them with refresh, width, feedback, and drive controls.

*This sits somewhere between resonator, spectral smear, and small synthetic reverb. The useful bit is that `Drive` and `Feedback` can push it from subtle colouring into much more unstable textures without needing an external IR library.*
